...
def validate_official(args, data_loader, model, batch_size_fixed):
    # we need batch_size_fixed because the old code was relying on the torch iterable to get the batch size,
    # but torch iterable gives a wrong batch size in the last iteration. It returns the number of elements left
    ...
    examples = 0
    trans_dict, sources, attentions_dict, attentions_transformer_dict = dict(), dict(), dict(), dict()
    attn_transformer_all_head_dict = dict() # MULTI HEAD
    # Fix: this ensure the batch size is fixed and this matches the assumptions
    # on ho the subsequent code threat the batch_size variable
    batch_size = batch_size_fixed # FIX
    with torch.no_grad():
        ...
        for batch_no, ex in enumerate(pbar):
            ...
            # bug: we assume that ex['batch_size'] stays constant, but unfortunately
            # torch gives a ex['batch_size'] that is equal to the remaing elements
            # for the last itertion and this creates unexpected ids that are colliding
            # with some ids we already used
            # batch_size = ex['batch_size'] # BUG
            ...
            translations, attentions, attentions_transformer, attn_transformer_all_head = \
                builder.from_batch(
                    ret,
                    ex['code_tokens'],
                    targets,
                    ex['src_vocab'])
            # (Pdb) attentions[1][0].shape
            # torch.Size([9, 150])
            # (Pdb) attentions[0][0].shape
            # torch.Size([12, 51])

            src_sequences = [code for code in ex['code_text']]
            for eid, trans, src, att, att_transformer, att_transformer_all_head \
                    in zip(ids, translations, src_sequences, attentions, attentions_transformer, attn_transformer_all_head):
                pdb.set_trace()
                #if eid == "14" or eid == 14:
                #    pdb.set_trace()
                trans_dict[eid] = trans
                sources[eid] = src
                attentions_dict[eid] = att
                attentions_transformer_dict[eid] = att_transformer
                attn_transformer_all_head_dict[eid] = att_transformer_all_head

            examples += batch_size

    hypotheses, references, attention_maps, attention_transformer_maps = dict(), dict(), dict(), dict()
    ...
    # EXTRACT COPY ATTENTION
    for eid, att in attentions_dict.items():
        #if eid == "14" or eid == 14:
        #    pdb.set_trace()
        attention_maps[eid] = torch.mean(att[0], dim=0).squeeze().tolist()

    # EXTRACT REGULAR (TRANSFORMER ATTENTION - ONLY ONE MUTIHEAD ATTENTION)
    for eid, att_transformer in attentions_transformer_dict.items():
        #if eid < 5:
            #tensor_to_reduce = att_transformer[0]
            #print(f'single attention - batch record: {eid}')
            #print(f'Dimension of tensor to reduce with mean: {tensor_to_reduce.shape}')
        attention_transformer_maps[eid] = torch.mean(att_transformer[0], dim=0).squeeze().tolist()



    # EXTRACT REGULAR (TRANSFORMER ATTENTION - ALL 8 MUTIHEAD ATTENTIONs)
    # create 8 empty lists
    attention_transformer_all_head_maps = []
    for i in range(8):
        attention_transformer_all_head_maps.append({})

    #import pdb
    #pdb.set_trace()

    # from 0..7 -> c_head_number
    for c_head_number, attention_transformer_single_head_map in enumerate(attention_transformer_all_head_maps):
        # from 0..77 -> eid
        for eid, att_transformer in attn_transformer_all_head_dict.items():
            tensor_to_reduce = att_transformer[0][c_head_number]
            #if c_head_number < 1 and eid < 5:
                #print(f'head: {c_head_number} - batch record: {eid}')
                #print(f'Dimension of tensor to reduce with mean: {tensor_to_reduce.shape}')
            attention_transformer_single_head_map[eid] = torch.mean(tensor_to_reduce, dim=0).squeeze().tolist()

            if c_head_number == 0:
                assert torch.equal(tensor_to_reduce, attentions_transformer_dict[eid][0])
                # check same first elements also after the reduction of the mean and the conversion to list
                assert attention_transformer_single_head_map[eid][0] == attention_transformer_maps[eid][0]

    print(attention_transformer_maps[1])
    print(attention_transformer_all_head_maps[0][1])

    #for c_head_number, attention_transformer_single_head_map in enumerate(attention_transformer_all_head_maps):
    #    for eid, att_transformer_all_head in attn_transformer_all_head_dict.items():
    #        if eid == 77:
    #            pdb.set_trace()
    #        attention_transformer_single_head_map[eid] = \
    #            torch.mean(att_transformer_all_head[0][c_head_number], dim=0).squeeze().tolist()

    #        if c_head_number == 0:
    #            assert torch.equal(att_transformer_all_head[0][c_head_number], attentions_transformer_dict[eid][0])

    # ADDITIONAL CHECK THAT THE SINGLE is EQUAL TO THE first of the 8 MULTIHEADED
    for a, b in zip(attention_transformer_maps[1], attention_transformer_all_head_maps[0][1]):
        assert a == b

    print('The attention weights of the first multi head attention can be found untouched also in the version that extracts 8 layers')
    ...
    if args.only_generate:
        ...
        with open(args.pred_file + '.attention_copy', 'w') as fw:
            #fw.write(str(attention_maps))
            json.dump(attention_maps, fw, indent=4)
        with open(args.pred_file + '.attention_transformer', 'w') as fw:
            #fw.write(str(attention_maps))
            json.dump(attention_transformer_maps, fw, indent=4)
        for c_head_number, attention_transformer_single_head_map in enumerate(attention_transformer_all_head_maps):
            with open(args.pred_file + f'.attention_transformer_{c_head_number}_only_transformer_decoder_side', 'w') as fw:
                json.dump(attention_transformer_single_head_map, fw, indent=4)
    else:
...
def main(args):
    ...
    # --------------------------------------------------------------------------
    # DO TEST
    validate_official(args, dev_loader, model, batch_size_fixed=args.test_batch_size)
